...
export default class Marker {
  constructor(shipName, shipType, trueheading, cog, sog, posX, posY, time, map, vectorSource) {
    ...
    let fill, stroke;
    sog > 30 ? fill = red : sog > 17 ? fill = orange : sog > 13 ? fill = yellow : sog > 9 ? fill = green
  ...
  changeLost(zoom) {
    let fill, svgUrl = '', svgShip = '', svgStop = ''
    if (this.lost) {
      svgShip = `<svg width="120" height="167" viewBox="0 0 120 167" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M11.117 152.487L60 16.282L108.883 152.487L63.0857 121.447L60 119.356L56.9143 121.447L11.117 152.487Z" 
        fill="#C90000" stroke="${this.stroke}" stroke-width="11"/></svg>`;
      svgStop = `<svg width="93" height="93" viewBox="0 0 93 93" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="46.5" cy="46.5" r="41.5" fill="#C90000" stroke="${this.stroke}" stroke-width="10"/></svg>`;
      this.sog < 2 ? svgUrl = `data:image/svg+xml;utf8,${encodeURIComponent(svgStop)}`
        : svgUrl = `data:image/svg+xml;utf8,${encodeURIComponent(svgShip)}`;
    ...
    this.feature.getStyle().setImage(new Icon({
      src: svgUrl,
      scale: this.calculateMarkerScale(zoom),
      rotation: this.cog * (Math.PI / 180),
      rotateWithView: true,
